...
Tenemos que descargar los ejecutables y moverlos a la carpeta bin poniendo un asterisco al final de data para que todo aquello que empiece por data se descarte
# Objetivos
...
Uno de los primeros comandos que le doy al dataset es summary, para que te explique. Datasets sirve para descargar cosas del NCBI, y le podemos pedir que descargue metadatos mediante summary, pueden ser entradas de datos diferentes (genes, taxonomia...)
Se puede ejecutar tal cual
Copiamos a partir de datasets trypanosoma para pedirle informacion sobre genomas del taxon Trypanosoma por ej y ademas le pido que esten anotados, es decir, con los elementos funcionales, y en el formato json lines que es un formato adecuado para dataformat
Todos estos datos necesitamoas transformarlo en una tabla y para ello sirve el comando dataformat
Tenemos que decirle que son datos de tipo genoma, y ademas que necesito info sobre la fecha de publicacion del ensamblaje, el nivel de ensamblaje (de cromosoma, de contig...). Podemos pedirle un monton de campos de informacion. Con el genome --help nos da mucha informacion sobre los campos
Lasch flags son las opciones, y pueden ser fields, una lista con los nombres de los campos que queremos descargar y son variables y caracteristicas de los genomas. Hay algunos que a lo mejor no tendran info de todo, pero al menos el nombre del organismo y el taxon...
```{bash}
...
La diferencia clave es el inicio GCA y GCF, aunque serían el mismo archivo
