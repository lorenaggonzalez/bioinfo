...
```
Lo que se observa en el primer bloque es que no funciona. En el segundo la URLencode(si se copia y pega en la barra del navegador me muestra la tabla de resultados y traduce automaticamente algun espacio que hay entre una condicion y otra y cuando le pedimos tambien genome representation las comillas las sabe diferenciar; en cambio en R con la funcion curl download se le facilita dando esa funcion curl download para no confundir y que no se detenga en un espacio por ejemplo). En el segundo bloque se nos descarga un archivo z1.text. El tercer bloque tambien funciona, con la tecla control y enter ejecuta la linea sobre la que se encuentra, la ejecuta y pasa a la siguiente. Lo que hace es juntar el contenido de la variable portal, de results y genera algo a lo que yo le aplico la función URLencode. Los espacios de AND genome los sustituye por un %20; las comillas con una barra delante; y una ultima sustitución por %23

El tercer bloque nos parece mejor, está todo mucho mas claro, es mś práctico. Sí quiero realizar una busqueda diferente, solo tengo que editar ese bloque y cambiarlo.

Estamos realizando los 5 pasos para realizar una consulta avanzada en el portal ENA. La función curl download lo que hace es guardar esos contenidos en un archivo de texto.
:::
...
query  <- 'query=tax_tree(32281) AND description="ADH" AND mol_type="genomic dna"&'
...
```
R cuandoo se topa con un apóstrofe, se lo toma como una comilla. Podemos decirle que no use ningun caracter para abrir comillas

Con conding no ha funcionado; ninguna en su descripción incluia la palabra alcohol dhase; en cambio con la palabra sequence sí.
En R todo son funciones, y lo que hacen es sustituir el valor

1. ¿Cuántas secuencias hay con el término "alcohol dehydrogenase" en su descripción?
He encontrado 547 líneas
2. ¿Qué longitudes tienen? 
La media de valores según base count es de 846
3. ¿De cuántas especies diferentes son estas secuencias?
Hay 137 valores unicos de la columna de "scientific name"
...
:::{.callout-note icon=false}
## Ejercicio 3
Este ejercicio está redactado como si la URL no estuviera modificado
...
Necesitamos la modificación y que se vea como aumenta la longitud de la secuencia y que se vea según las fechas a la más reciente.
:::
```{r}
plat <- platelmintos
